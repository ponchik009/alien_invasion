...
class AlienInvasion:
  ...
  def run_game(self):
    ...
    while True:
      ...
      if self.stats.game_active:
        ...
        # Обновляет положение флота пришельцев
        self._update_aliens()
  ...
  def _start_game(self):
    """Сбрасывает статистику и запускает игру, если она неактивна"""
    if not self.stats.game_active:
      ...
      # Очистки панели счёта, уровня и жизней
      self.sb.prep_score()
      self.sb.prep_level
      self.sb.prep_ships()
  ...
  def _create_fleet(self):
    ...
    number_aliens_x = (avaliable_space_x + alien_width) // (2 * alien_width)
    
    # Определение количества рядов, помещающихся на экране
    ship_height = self.ship.rect.height
    avaliable_space_y = (self.settings.screen_height - ship_height - (5 * alien_height))
  ...
  def _create_alien(self, alien_number: int, row_number: int):
    ...
    # Определение X координаты
    alien_width = alien.rect.width
    alien.x = alien_width + 2 * alien_width * alien_number
    alien.x_relative = alien.x / (self.settings.screen_width - alien.rect.width)
    alien.rect.x = alien.x
    
    # Определение Y координаты
  ...
  def _ship_hit(self):
    """Обрабатывает столкновение корабля с прилешьцем"""
    if self.stats.ships_left > 0:
      # Уменьшение количества оставшихся кораблей и обновление панели жизней
